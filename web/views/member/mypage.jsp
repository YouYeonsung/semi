					...
					<div class="grid-body">
						...
						function updatePoint(){
							...
							*/
							...
							//가격
							buyer_email: '<%=m1.getEmail()%>',
							buyer_name: '<%=m1.getUserName()%>',
							buyer_tel: '<%=m1.getPhone()%>',
							buyer_addr: '<%=(m1.getAddress()).replaceAll("subaddr", " ")%>',
							...
							*/
							...
							if (rsp.success) {
								...
								location.href="<%=request.getContextPath()%>/member/updatePoint?point="+point+"&userId=<%=m1.getUserId()%>";
								return point;
							} else {
								var msg = '결제에 실패하였습니다.';
